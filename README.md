...
Paid enrollment is disabled by default with `BILLING_ENABLED=false`. Visitors can request private-launch access from the Invite-only tab on `/pricing`. Requests enter the Admin queue, where an administrator can approve and email a one-time Premium link through Resend or decline the request. Administrators can also generate standalone links, set a redemption deadline, and revoke them later. Invitations are not bound to an email address; the first authenticated user to redeem one receives access. Only a SHA-256 hash of the token is stored. Redemption is performed by a row-locking PostgreSQL function, so concurrent requests cannot use the same invitation twice.
