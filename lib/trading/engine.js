...
function buildIndicatorsFromQuote(quote) {
  const entryPrice = Number(quote.c);
  const prevClose = Number(quote.pc || entryPrice);
  const high = Number(quote.h || entryPrice * 1.01);
  const low = Number(quote.l || entryPrice * 0.99);
  const open = Number(quote.o || prevClose);
  const changePct = Number(quote.dp || 0);
  const intradayPct = open ? ((entryPrice - open) / open) * 100 : changePct;
  const rangePct = high > low ? (entryPrice - low) / (high - low) : 0.5;
  const dayRangePct = entryPrice ? ((high - low) / entryPrice) * 100 : 1.2;
  const trendBias = changePct * 0.65 + intradayPct * 0.35;
  const rsiEstimate = 38 + rangePct * 26 + trendBias * 1.9;
  const rsi = +Math.max(15, Math.min(85, rsiEstimate)).toFixed(1);
  const macdHist = +(trendBias * 0.03 + (rangePct - 0.5) * 0.08).toFixed(4);
  const bbPct = +Math.max(0.08, Math.min(0.92, 0.2 + rangePct * 0.6 + trendBias / 18)).toFixed(3);
  const stochK = +Math.max(0, Math.min(100, rangePct * 100 + intradayPct * 2)).toFixed(1);
  const atr = Math.max(high - low, entryPrice * Math.max(0.008, dayRangePct / 100));
  const hv = +(Math.abs(changePct) * 10 + dayRangePct * 6 + 12).toFixed(1);
  const volRatio = +Math.max(0.75, Math.min(2.8, 0.85 + Math.abs(changePct) * 0.22 + dayRangePct * 0.18)).toFixed(2);
  const sma50 = +(entryPrice / (1 + trendBias / 220 || 1)).toFixed(2);
  const sma200 = +(entryPrice / (1 + trendBias / 420 || 1)).toFixed(2);
  const above50 = entryPrice >= sma50;
  const above200 = entryPrice >= sma200;
  ...
  return {
    ...
    above50,
    above200,
    goldenCross: above50 && above200 && trendBias > 0.9 && rangePct > 0.55,
    deathCross: !above50 && !above200 && trendBias < -0.9 && rangePct < 0.45,
    ivPct: hv,
    ivRank: Math.min(80, Math.abs(changePct) * 8 + dayRangePct * 4 + 10),
    adx: +(18 + Math.abs(rsi - 50) * 0.55 + Math.abs(trendBias) * 1.5).toFixed(0),
    trendStrength: Math.abs(trendBias) > 2 ? 'STRONG' : Math.abs(trendBias) > 0.75 ? 'MODERATE' : 'WEAK',
    dataMode: 'QUOTE ONLY',
  };
}

function buildIndicatorsFromCandles(quote, candles) {
...
function score(inds, strategy = 'momentum', regime = 'neutral') {
  ...
  if (inds.dataMode === 'QUOTE ONLY') {
    const quoteTilt = Math.max(
      -12,
      Math.min(
        12,
        (inds.rsi - 50) * 0.18 +
    ...
    if (quoteTilt > 0.25) bull += +quoteTilt.toFixed(1);
    if (quoteTilt < -0.25) bear += +Math.abs(quoteTilt).toFixed(1);
  }
...
export async function generateSignal(symbol, strategy = 'momentum', options = {}) {
  const sym = symbol.trim().toUpperCase();
  const to = Math.floor(Date.now() / 1000);
  const from = to - 120 * 86400;
  const { includeProfile = true } = options || {};

  const [quoteResult, candleResult, profileResult] = await Promise.allSettled([
    fetchQuote(sym),
    fetchCandles(sym, 'D', from, to),
    includeProfile ? fetchProfile(sym) : Promise.resolve(null),
  ]);
  ...
  const profile = includeProfile && profileResult.status === 'fulfilled' ? profileResult.value : null;
  const hasCandles = candles && candles.s === 'ok' && Array.isArray(candles.c) && candles.c.length >= 20;

  const entryPrice = quote.c;
  const indicators = hasCandles ? buildIndicatorsFromCandles(quote, candles) : buildIndicatorsFromQuote(quote);
  const scored = score(indicators, strategy);
  const confidencePenalty = indicators.dataMode === 'QUOTE ONLY' ? 8 : 0;
  const adjustedConfidence = Math.max(scored.action === 'HOLD' ? 38 : 45, scored.rawConfidence - confidencePenalty);
  ...
  return {
    ...
    rawConfidence: Math.round(adjustedConfidence),
    entryPrice,
    stopLoss,
    targetPrice,
    indicators,
