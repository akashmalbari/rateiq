...
function buildIndicatorsFromQuote(quote) {
  ...
  const rsiEstimate = 35 + rangePct * 30 + changePct * 1.5;
  const rsi = +Math.max(15, Math.min(85, rsiEstimate)).toFixed(1);
  const macdHist = +(((entryPrice - prevClose) / (prevClose || entryPrice || 1)) * 5).toFixed(4);
  const bbPct = +Math.max(0.1, Math.min(0.9, 0.5 + changePct / 10)).toFixed(3);
  const stochK = +(rangePct * 100).toFixed(1);
  const atr = Math.max(high - low, entryPrice * 0.015);
  const hv = +(Math.abs(changePct) * 10 + 15).toFixed(1);
  const sma50 = +(entryPrice / (1 + changePct / 500 || 1)).toFixed(2);
  const sma200 = +(entryPrice / (1 + changePct / 1000 || 1)).toFixed(2);
  ...
  return {
    ...
    sma200,
    above50: changePct > -2,
    above200: changePct > -5,
    goldenCross: changePct > 0.5,
    deathCross: changePct < -0.5,
    ivPct: hv,
    ivRank: Math.min(80, Math.abs(changePct) * 8 + 10),
    adx: +(20 + Math.abs(rsi - 50) * 0.5).toFixed(0),
    trendStrength: Math.abs(changePct) > 2 ? 'STRONG' : Math.abs(changePct) > 0.5 ? 'MODERATE' : 'WEAK',
    dataMode: 'QUOTE ONLY',
  };
}
