...
async function supabase(path, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(`Supabase error: ${response.status} ${bodyText}`);
  }
  ...
  try {
    ...
    throw new Error(`Supabase returned non-JSON response (${response.status}): ${bodyText}`);
  }
}
...
export async function getSignalStats(symbol, strategy) {
  ...
  const winRate = sampleSize ? (wins / sampleSize) * 100 : null;
...
export async function upsertSiteContent(key, content) {
  ...
  return supabase(`site_content?${params.toString()}`, {
    method: 'POST',
...
export async function getTradingUserByEmail(email) {
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedEmail) return null;

  const params = new URLSearchParams({
    email: `eq.${normalizedEmail}`,
    select: 'id,email,display_name,password_hash,password_salt,role,status,created_at,last_login_at',
    limit: '1',
  });

  const rows = await supabase(`trading_users?${params.toString()}`, { method: 'GET' });
  return rows?.[0] || null;
}

export async function createTradingUser({ email, displayName, passwordHash, passwordSalt, role = 'member' }) {
  const normalizedEmail = normalizeEmail(email);
  const payload = {
  ...
  const rows = await supabase('trading_users', {
    method: 'POST',
    headers: { Prefer: 'return=representation' },
    body: JSON.stringify(payload),
  });
...
export async function updateTradingUserLastLogin(id) {
  const params = new URLSearchParams({ id: `eq.${id}` });
  return supabase(`trading_users?${params.toString()}`, {
    method: 'PATCH',
    headers: { Prefer: 'return=minimal' },
    body: JSON.stringify({
